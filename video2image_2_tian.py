...
def parse_args():
    ...
    parser.add_argument('--video_file', type=str,
                        help='file containing the slot2_path of the videos')
    # parser.add_argument('--out_path', type=str,
    #                     help='output directory for the sliced frames')
    ...
    if decode_type == 'ffmpeg':
        ...
        if ret:
            ...
            if se is None:
                ...
                #     cmd = 'ffmpeg -i \"{}\"  -threads 1 -vf scale={}:-1 -q:v 1 \
                #         \"{}/{}\"'.format(src, se, video_folder, prefix)
                cmd = 'ffmpeg -i \"{}\"  -threads 1 -vf scale={}:{} -q:v 1 \
                        \"{}/{}\"'.format(src, 256, 340, video_folder, prefix)                
...
def main():
    ...
    print('video_file: ', args.video_file)
    ...
    for vid, info in data.items():
        if info['slot2_path'] == '':
            ...
            src_list.append(info['slot2_path'])
            dest_list.append(info['slot2_framepath'])

    print('len(src_list): ', len(src_list))
    print('len(dest_list): ', len(dest_list))
